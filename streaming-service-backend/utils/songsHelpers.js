...
const validDate = (date, param)=>{
    ...
    //leap year
    if(Number(date[0]) == 2 && Number(date[1])>28 && Number(date[2])%4 !=0 ){
...
const validURL = (url)=>{
    let regexURL = /^(?:https?|ftp):\/\/[\w\-]+(?:\.[\w\-]+)+(?:[\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?/;
...
export default {validGenre, ObjectIdtoString,  unAuthorizedWrapper, emptyValidation, notFoundWrapper, badUserInputWrapper, validDate, validURL, serverSideErrorWrapper, validObjectId};