...
const songSchema = new mongoose.Schema({
  ...
  song_url: {
    ...
    validate: {
      validator: (url)=> {
        return /^(?:https?|ftp):\/\/[\w\-]+(?:\.[\w\-]+)+(?:[\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?/.test(url);
      },
  ...
  producers: {
    ...
    validate: {
      ...
      message: () => `Invalid array of strings. Please make sure it's not empty, doesn't contain only spaces, and contains only letters.`
    }
  },
  language: {
    type: String,
