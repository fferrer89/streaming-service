...
const exportedMethods = {
    checkId(id, varName) {
        ...
        if (!ObjectId.isValid(id)) throw `Error: invalid object ID for ${varName} field`;
        return id;
    },
    ...
    isoToUsDateFormat(isoDate, varName){
        ...
        if (dateParts.length !== 3) {
            throw new RangeError(`${varName} must be a in ISO format (e.g.: 2024-04-17`);
        }
    ...
    usToIsoDateFormat(usDate, varName){
        ...
        if (dateParts.length !== 3) {
            throw new RangeError(`${varName} must be a in US format (e.g.: 04/17/2024`);
        }
