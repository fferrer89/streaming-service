...
const httpClientReqs = {
    uploadFile: async (file) => {
        ...
        try {
            ...
            throw new Error('Error making the fetch request - Internal Server Error', { cause: e });
        }
        try {
            responseBody = await response?.json();
        } catch (e) {
            throw new Error(`Error parsing the response to JSON - Internal Server Error | ${await response?.text()?.toString()}`, { cause: e });
        }
        if (!response?.ok) {
            // It fetch() return an ErrorBoundary with (Status Code of 400-500)
            if (response?.status === 400) {
                throw new Error(`Error making the fetch request - ${responseBody?.message}`, { cause: responseBody });
            } else {
                throw new Error(`Error making the fetch request - ${responseBody?.message}`, { cause: responseBody });
            }
        }
