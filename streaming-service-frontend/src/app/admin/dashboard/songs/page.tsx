...
const SongList: React.FC = () => {
  ...
  const { data, loading, error } = useQuery(queries.GET_DASHBOARD_SONGS, { fetchPolicy: 'cache-and-network' });
  ...
  const paginatedSongs = data.songs.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
          ...
          <div className='flex flex-col gap-8 py-10 px-6 w-full h-full sm:ml-60'>
            <div className='flex justify-between items-center'>
              ...
              <div className="inline-flex gap-4">
                ...
                >Prev</button>
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={data.songs.length / PAGE_SIZE <= currentPage}
                  className="bg-transparent hover:bg-[#22333B] text-[#22333B] font-semibold hover:text-[#C6AC8E] py-2 px-4 border border-[#22333B] hover:border-transparent rounded"
                >Next</button>
              </div>
            </div>
            <div className='flex flex-col md:flex-wrap md:flex-row gap-6 w-full'>
              {(paginatedSongs.length > 0) ?
                (paginatedSongs.map((song: Songs) => (
                  <div key={song._id} className='flex flex-col sm:w-56 items-center justify-center text-center px-3 py-6 rounded-md bg-[#22333B]'>
                    {(song.cover_image_url) ?
                      <Image src={`http://localhost:4000/file/download/${song.cover_image_url}`} alt='Song Cover' width={100} height={100} className='mb-4 rounded-full' /> :
                      <PiMusicNotesFill className='w-[100px] h-[100px] mb-4' />
                    }
                    <h5 className='mb-2 text-xl font-medium text-[#C6AC8E] text-pretty'>{song.title}</h5>
                    <span className='text-sm mb-2 text-[#C6AC8E]'>Likes: {(song.likes) ? song.likes : '0'}</span>
