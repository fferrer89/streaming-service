  ...
  return (
    <>
      <div className={`fixed inset-0 flex justify-center items-center
        ...
        <div onClick={(event) => event.stopPropagation()} className={`bg-white rounded-sm shadow p-6 transition-all
                        ${open ? 'scale-100 opacity-100' : 'scale-125 opacity-0'}`}>
          <button className='absolute top-2 right-2 p-2' onClick={onClose}>
            <CgCloseR fontSize={24} className='hover:text-red-500'></CgCloseR>
          </button>
          <div className='flex flex-col justify-center items-center text-center w-60'>
            <FaTrashAlt fontSize={36} color='red'></FaTrashAlt>
            <div className='mt-6'>
